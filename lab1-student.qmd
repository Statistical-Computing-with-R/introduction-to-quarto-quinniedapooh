...
#| fig-align: "center"
#| fig-cap: 
#|   - "Box plot comparing tooth length between supplement delivery methods"
#|   - "Box plot comparing tooth length between dosages uses"
...
## Two-sample independent t-test comparing tooth length between OJ & VC supplement delivery methods
```{r}
t.test(len ~ supp, data = ToothGrowth, alternative = "two.sided", var.equal = FALSE)

```
1. Given the p-value of 0.06063 and assuming a significance level of 0.05, we fail to reject the null hypothesis that the treatment mean tooth length for the OJ supplement delivery method is the same as the treatment mean tooth length from the VC supplement delivery method. This means that we lack sufficient evidence to find a  statistically significant difference in treatment mean tooth length between the OJ and VC supplement delivery methods given a significance level of 0.05.  
2. With a default confidence level of 95 percent, this means that we are 95 percent confident that the true difference in treatment mean tooth lengths between OJ and VC supplement delivery methods lies within the interval of (-0.1710156, 7.5710156). Since the value of 0 lies within this interval, the difference between the treatment mean tooth lengths could be 0, which further supports our failure to reject the null hypothesis.
